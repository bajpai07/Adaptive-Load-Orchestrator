...
6. **Portfolio Demo Pre-Seeding of 2 Simulated Neighborhood Members**:
   - *Rationale*: For demo/portfolio-viewing purposes, 2 simulated neighborhood members (`Aarav Mehta` • `Flat 402` and `Priya Sharma` • `Flat 201`) are pre-seeded into every new Group Cart and Rider Trip on startup and simulation loop restart. This ensures that a first-time visitor opening the live URL immediately sees an active multi-member trip without needing to open multiple browser tabs. In a real production deployment, members would only ever be genuine customer orders — this seeding exists purely to make the live demo URL self-explanatory for portfolio reviewers. When a visitor joins, they attach as the 3rd member ($N=3$), unlocking 100% FREE delivery (₹0.00) via genuine mathematical tier calculation.
